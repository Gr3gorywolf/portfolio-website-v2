...
export function ProjectCard({ project, lastCommit, stats }: ProjectCardProps) {
                                ...
                                {project.gallery.slice(1, 3).map((image, index) => (
                                    <div key={index} className="relative">
                                        <img
                                            ...
                                            className="object-cover max-h-[94px] w-full"
                ...
                {lastCommit && (
                    <div className="mb-4 p-2 bg-muted/50 rounded text-xs mt-auto">
                        <div className="flex items-center gap-2">
                            <img
                                className="w-4 h-4 bg-accent-orange rounded-full"
                                src={
                                    lastCommit.committer?.avatar_url ??
                                    getGravatarUrl(lastCommit.commit.committer?.email)
                                }
                                alt={`${lastCommit.committer?.login} avatar`}
                            />
                            <span className="font-medium">
                                {truncateString(lastCommit.committer?.login ?? lastCommit.commit?.author?.name, 13)}
                            </span>
                            <span className="text-muted-foreground">
                                {humanReadableDate(lastCommit.commit?.committer.date)}
                            </span>
                        </div>
